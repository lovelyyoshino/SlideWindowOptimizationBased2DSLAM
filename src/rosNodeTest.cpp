...
void feature_callback(const slam_data_sim::RangeBearingObsDataPtr &features_msg)
{
    ...
    estimator.inputFeature(features_msg->header.stamp.toSec(), observations); // 加载特征
}
...
void odom_callback(const nav_msgs::OdometryPtr &odom_msg)
{
    ...
    double dth = Utility::normalizeAngle(tf::getYaw(odom_msg->pose.pose.orientation) - tf::getYaw(last_odom.pose.pose.orientation)); // 设置差值
